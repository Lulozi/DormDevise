...
class DoorWidgetPinReceiver : BroadcastReceiver() {
    ...
    private fun resolveWidgetIds(
        ...
        val existingWidgetIds = intent?.getIntArrayExtra(
            DoorWidgetPinRequestHelper.extraExistingWidgetIds,
        )
        if (existingWidgetIds != null) {
            val existing = existingWidgetIds.toSet()
            return currentWidgetIds.filterNot { it in existing }.toIntArray()
        }

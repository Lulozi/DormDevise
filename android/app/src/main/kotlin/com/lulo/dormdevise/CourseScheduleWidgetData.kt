...
internal data class CourseScheduleWidgetSnapshot(
    ...
    val sectionTimesByIndex: Map<Int, CourseScheduleSectionTime>,
    val breakDurationMillisByEndSection: Map<Int, Long>,
...
internal data class CourseScheduleSectionTime(
    val index: Int,
...
internal object CourseScheduleWidgetData {
    ...
    private const val DEFAULT_REMINDER_MINUTES = 90
    // 95 作为“自动提醒”保留值，不再允许更大提醒窗口。
    private const val MAX_REMINDER_MINUTES = 95
    ...
    fun buildSnapshot(
        ...
        val reminderMinutes = widgetData
            ...
            .coerceIn(0, MAX_REMINDER_MINUTES)
        ...
        val sectionTimesByIndex = buildSectionTimesByIndex(sections)
        val breakDurationMillisByEndSection = buildBreakDurationMillisByEndSection(sections)
        ...
        return CourseScheduleWidgetSnapshot(
            ...
            sectionTimesByIndex = sectionTimesByIndex,
            breakDurationMillisByEndSection = breakDurationMillisByEndSection,
    ...
    fun computeNextRefreshAtMillis(
        ...
        var hasUpcomingOrOngoingCourse = false
        ...
        snapshot.courses.forEach { item ->
            ...
            if (startMillis != null && startMillis > nowMillis) {
                hasUpcomingOrOngoingCourse = true
            ...
            // 连堂课程按节次边界精确刷新，避免“课间休息/进行中”切换滞后。
            if (item.sectionCount > 1) {
                for (sectionIndex in item.startSection until item.endSection) {
                    val currentSection = snapshot.sectionTimesByIndex[sectionIndex]
                    val nextSection = snapshot.sectionTimesByIndex[sectionIndex + 1]
                    val sectionEndMillis = currentSection
                        ?.let { section -> parseTodayTimeMillis(section.end, now) }
                    val nextSectionStartMillis = nextSection
                        ?.let { section -> parseTodayTimeMillis(section.start, now) }

                    if (sectionEndMillis != null && sectionEndMillis > nowMillis) {
                        hasUpcomingOrOngoingCourse = true
                        nextMillis = minOfNullable(nextMillis, sectionEndMillis)
                    }
                    if (nextSectionStartMillis != null && nextSectionStartMillis > nowMillis) {
                        hasUpcomingOrOngoingCourse = true
                        nextMillis = minOfNullable(nextMillis, nextSectionStartMillis)
                    }
                }
            }
        }

        // 当天仍有课程相关状态变化时，至少每分钟刷新一次，保证倒计时与状态显示及时。
        if (hasUpcomingOrOngoingCourse) {
            nextMillis = minOfNullable(nextMillis, nextMinuteTick)
        }
                ...
                for (index in 0 until array.length()) {
                    ...
                    add(
                        CourseScheduleSectionTime(
                            index = index + 1,
                            segmentName = obj.optString("segment", ""),
    ...
    private fun buildSectionTimesByIndex(
        sections: List<CourseScheduleSectionTime>,
    ): Map<Int, CourseScheduleSectionTime> {
        return sections.associateBy { it.index }
    }

    private fun buildBreakDurationMillisByEndSection(
        sections: List<CourseScheduleSectionTime>,
    ): Map<Int, Long> {
        if (sections.size < 2) return emptyMap()

        return buildMap {
            for (index in 0 until sections.lastIndex) {
                val current = sections[index]
                val next = sections[index + 1]
                // 仅在同一教学时段内认定“课间休息”，跨时段间隔（如午休）不作为小课间。
                if (current.segmentName.isNotBlank() &&
                    next.segmentName.isNotBlank() &&
                    current.segmentName != next.segmentName
                ) {
                    continue
                }

                val currentEndMinutes = parseTimeToMinutes(current.end) ?: continue
                val nextStartMinutes = parseTimeToMinutes(next.start) ?: continue
                val breakMinutes = nextStartMinutes - currentEndMinutes
                if (breakMinutes > 0) {
                    put(index + 1, breakMinutes * MINUTE_MILLIS)
                }
            }
        }
    }

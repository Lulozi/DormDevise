...
class MainActivity : FlutterActivity() {
		...
		MethodChannel(
			...
			when (call.method) {
				"shareImageToPackage" -> {
					...
					val fileName = call.argument<String>("fileName") ?: "shared_image.png"
					val mimeType = call.argument<String>("mimeType") ?: "image/png"
					val packageName = call.argument<String>("packageName") ?: ""
					val text = call.argument<String>("text")

					if (bytes == null || bytes.isEmpty() || packageName.isBlank()) {
						result.error("INVALID_ARGS", "分享参数不完整", null)
						return@setMethodCallHandler
					}

					runOnUiThread {
						try {
							val shareUri = createShareImageUri(bytes, fileName)
							...
							result.error("DIRECT_SHARE_FAILED", e.message, null)
						}
					}
				}
				else -> result.notImplemented()
	...
	/**
	 * 将图片字节写入缓存目录并生成可跨应用读取的 content Uri。
	 */
	private fun createShareImageUri(bytes: ByteArray, fileName: String): Uri {
		val shareDir = File(cacheDir, "direct_share_images")
		if (!shareDir.exists()) {
			shareDir.mkdirs()
		}

...
class _SettingsOpenContainerRoute<T> extends ModalRoute<T> {
  ...
  void close() {
    ...
    if (routeContext == null || !isActive) {
      return;
    }
    // 延迟到下一帧执行，避免在导航器锁定阶段重入 pop。
    SchedulerBinding.instance.addPostFrameCallback((_) {
      final BuildContext? latestContext = subtreeContext;
      if (latestContext == null || !isActive || !isCurrent) {
        return;
      }
      Navigator.of(latestContext).pop();
    });
  }

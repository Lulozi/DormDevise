...
class _DoorWidgetSettingsPageState extends State<DoorWidgetSettingsPage>
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: TabBarView(
        controller: _tabController,
        children: const [_ScheduleWidgetTab(), _DoorWidgetTab()],
      ),
    );
  }
}
                  ...
                  ExpandableItem(
                    ...
                    value: Text(
                      _reminderMinutes == 0
                          ? '不提醒'
                          : _reminderMinutes ==
                                CourseWidgetDisplaySettings.maxReminderMinutes
                          ? '自动'
                          : '$_reminderMinutes 分钟',
                      style: TextStyle(
                        fontSize: 14,
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: const [
                      ...
                      SizedBox(height: 8),
                      Text('1. 点击上方按钮或长按桌面空白处添加组件。'),
                      Text('2. 课表组件会自动读取当前正在使用的课程表。'),
                      Text('3. 修改课程、周次配置或切换课表后，桌面组件会自动同步。'),
                      Text('4. 组件支持左右切换日期查看课程，当前日期会有高亮显示。'),
                      Text('5. 组件支持上下滑动查看全天课程，点击课程后会直接显示完整课程。'),
                    ],
                  ),
                ),
              ),
...
class _CourseScheduleWidgetPreview extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    const String upcomingHint = '课间休息 · 3-4节';
...
class _CoursePreviewRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final double resolvedDetailSize = (resolvedTitleSize - 2)
        .clamp(8, 18)
        .toDouble();
    // 预览卡片使用课程色半透明底，贴近真实组件视觉语义。
    final Color tintedCardColor = Color.alphaBlend(
      color.withValues(alpha: 0.44),
      colorScheme.surface,
    );
    final Color tintedBorderColor = color.withValues(alpha: 0.62);
    final Color indicatorColor = color.withValues(alpha: 0.92);
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(color: tintedBorderColor, width: 0.9),
      ),
      child: Row(
        children: [
          Container(

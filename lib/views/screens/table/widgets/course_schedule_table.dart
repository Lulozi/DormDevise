...
class CourseScheduleTable extends StatefulWidget {
  ...
  /// 课程数据代次，用于缓存课表块计算，减少键盘/弹窗动画期间的重复重建成本。
...
class _CourseScheduleTableState extends State<CourseScheduleTable>
  ...
  _CourseBlockCacheKey? _blockCacheKey;
  List<_CourseBlock>? _blockCache;
  ...
  Widget build(BuildContext context) {
    return LayoutBuilder(
      builder: (BuildContext context, BoxConstraints constraints) {
        ...
        final List<_CourseBlock> blocks = _resolveBlocks(columnMap);
  ...
  List<_CourseBlock> _resolveBlocks(Map<int, int> columnMap) {
    final _CourseBlockCacheKey cacheKey = _CourseBlockCacheKey(
      currentWeek: currentWeek,
      showNonCurrentWeek: showNonCurrentWeek,
      weekdayIndexesHash: Object.hashAll(weekdayIndexes),
      contentGeneration: widget.contentGeneration,
    );
    if (_blockCacheKey == cacheKey && _blockCache != null) {
      return _blockCache!;
    }
    final List<_CourseBlock> blocks = _buildBlocks(columnMap);
    _blockCacheKey = cacheKey;
    _blockCache = blocks;
    return blocks;
  }
...
class _CourseBlockCacheKey {
  const _CourseBlockCacheKey({
    required this.currentWeek,
    required this.showNonCurrentWeek,
    required this.weekdayIndexesHash,
    required this.contentGeneration,
  });

  final int currentWeek;
  final bool showNonCurrentWeek;
  final int weekdayIndexesHash;
  final int contentGeneration;
  ...
  bool operator ==(Object other) {
    ...
    return other is _CourseBlockCacheKey &&
        other.currentWeek == currentWeek &&
        other.showNonCurrentWeek == showNonCurrentWeek &&
        other.weekdayIndexesHash == weekdayIndexesHash &&

...
class DoorWidgetService {
  ...
  static const String _pendingAutoOpenKey = 'door_widget_pending_auto_open';
  ...
  static const Duration _manualSuccessDisplayDuration = Duration(seconds: 3);
  ...
  Future<void> initialize() async {
    ...
    if (!_promptChannelRegistered) {
      ...
      promptChannel.setMethodCallHandler((call) async {
        if (call.method == 'performAutoOpen') {
          debugPrint('桌面微件直开: 收到触发请求');
          final bool triggered = await triggerAutoOpenFromWidget();
          if (!triggered) {
            debugPrint('桌面微件直开: 本次请求已忽略（忙碌/防抖/无待处理标记）');
          }
    ...
    try {
      await triggerAutoOpenFromWidget(onlyIfPending: true);
    } catch (_) {}
  ...
  /// 统一执行桌面微件直开逻辑，避免 promptReady 与 pending 兜底并发重复触发。
  Future<bool> triggerAutoOpenFromWidget({bool onlyIfPending = false}) async {
    final Future<bool>? inFlight = _autoOpenInFlight;
    if (inFlight != null) {
      return inFlight;
    }
    final Future<bool> task = _triggerAutoOpenFromWidgetInternal(
      onlyIfPending: onlyIfPending,
    );
    _autoOpenInFlight = task;
    try {
      return await task;
    } finally {
      if (identical(_autoOpenInFlight, task)) {
        _autoOpenInFlight = null;
      }
    }
  }

  Future<bool> _triggerAutoOpenFromWidgetInternal({
    required bool onlyIfPending,
  }) async {
    await _ensureLoaded();
    final SharedPreferences prefs = await SharedPreferences.getInstance();
    final bool pending = prefs.getBool(_pendingAutoOpenKey) ?? false;
    if (onlyIfPending && !pending) {
      return false;
    }
    if (pending) {
      await prefs.setBool(_pendingAutoOpenKey, false);
    }
    if (_state.busy) {
      return false;
    }
    ...
    final DateTime? deadline = _manualResultDeadline;
    ...
    final DoorTriggerResult result = await DoorTriggerService.instance
        .triggerDoor();
    debugPrint('桌面微件直开: ${result.success ? '成功' : '失败'}，原因：${result.message}');

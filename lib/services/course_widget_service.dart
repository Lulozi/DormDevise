...
class CourseWidgetDisplaySettings {
  ...
  // 95 作为“自动提醒”保留值，不再允许超过该值。
  static const int maxReminderMinutes = 95;
  static const int reminderStepMinutes = 5;

  static const int defaultHeaderFontSize = 18;
  static const int defaultContentFontSize = 14;
  // 默认为“自动”。
  static const int defaultReminderMinutes = 95;
...
class CourseWidgetService {
  ...
  bool _pendingAutoSyncResetToToday = false;

  /// 初始化服务
  Future<void> initialize() async {
    if (_initialized) return;
    _initialized = true;
    _bindCourseDataChanges();
    await syncWidget();
  }

  /// 绑定课表数据变更流，统一自动刷新桌面组件。
  void _bindCourseDataChanges() {
  ...
  void _handleCourseDataChanged(CourseDataChangeEvent event) {
    if (!_shouldAutoSyncForEvent(event.scope)) {
      return;
    }
    if (_shouldResetDisplayDateForEvent(event.scope)) {
      _pendingAutoSyncResetToToday = true;
    }

    // 对连续保存操作进行去抖，避免一次设置触发多次组件刷新。
    _autoSyncDebounceTimer?.cancel();
    _autoSyncDebounceTimer = Timer(const Duration(milliseconds: 220), () {
      final bool resetToToday = _pendingAutoSyncResetToToday;
      _pendingAutoSyncResetToToday = false;
      unawaited(syncWidget(resetDisplayDateToToday: resetToToday));
    });
  }

  bool _shouldAutoSyncForEvent(String scope) {
    switch (scope) {
      case 'courses':
      case 'config':
      case 'semester_start':
  ...
  bool _shouldResetDisplayDateForEvent(String scope) {
    switch (scope) {
      case 'courses':
      case 'config':
      case 'semester_start':
  ...
  Future<void> syncWidget({bool resetDisplayDateToToday = false}) async {
    try {
      ...
      final snapshot = await CourseService.instance.loadScheduleSnapshot();
      final semesterStart = snapshot.semesterStart;
      final tableName = snapshot.tableName;
      final allCourses = snapshot.courses;
          ...
          sections
              .map(
                (section) => <String, String>{
                  // 同步教学时段名称，供 Android 组件按实际课间配置判定“课间休息”。

...
class UpdateCheckService {
  ...
  Future<UpdateDialogAction> showUpdateAvailableDialog(
    ...
    final String? displayDescription =
        ...
        ? null
        : normalizeReleaseMarkdownForGitHubDisplay(
            description,
            releasePageUrl: result.latestRelease.htmlUrl,
          );
            ...
            child: Column(
              ...
              children: <Widget>[
                ...
                if (displayDescription != null &&
                    displayDescription.isNotEmpty) ...<Widget>[
                  const SizedBox(height: 16),
                  MarkdownBody(
                    data: displayDescription,
                    // 使用 GitHub 风格扩展，尽量与发布说明原始语法保持一致。
                    extensionSet: md.ExtensionSet.gitHubWeb,
                    selectable: true,
                    onTapLink: (String text, String? href, String title) {
                      unawaited(
                        _openReleaseNotesLink(
                          dialogContext,
                          href,
  ...
  Future<void> _openReleaseNotesLink(
    BuildContext context,
    String? href,
    String? releasePageUrl,
  ) async {
    // 统一处理更新说明中的外链点击，失败时给出轻量反馈。
    final String raw = href?.trim() ?? '';
    if (raw.isEmpty) {
      return;
    }
    final Uri? releasePageUri = Uri.tryParse(releasePageUrl ?? '');
    final Uri? parsedUri = Uri.tryParse(raw);
    final Uri uri;
    if (parsedUri != null && parsedUri.hasScheme) {
      uri = parsedUri;
    } else {
      if (raw.startsWith('#') && releasePageUri != null) {
        uri = releasePageUri.replace(fragment: raw.substring(1));
      } else if (releasePageUri != null) {
        uri = releasePageUri.resolve(raw);
      } else {
        final Uri fallbackBase = Uri.parse(
          'https://github.com/Lulozi/DormDevise/',
        );
        uri = fallbackBase.resolve(raw);
      }
    }
    ...
    if (!launched && context.mounted) {
      ...
      messenger?.showSnackBar(const SnackBar(content: Text('链接打开失败')));
    }
  }
...
String normalizeReleaseMarkdownForGitHubDisplay(
  String markdown, {
  String? releasePageUrl,
}) {
  // 先统一换行，再做 GitHub 常见链接的文案改写，尽量贴近网页端展示。
  final String normalized = markdown.replaceAll('\r\n', '\n');
  final String withCompareLinks = _rewriteGitHubCompareLinks(normalized);
  final String withPullLinks = _rewriteGitHubPullRequestLinks(withCompareLinks);
  final String withUserMentions = _rewriteGitHubUserMentions(withPullLinks);
  return _rewriteGitHubShorthandPullReferences(
    withUserMentions,
    releasePageUrl: releasePageUrl,
  );
}

String _rewriteGitHubUserMentions(String markdown) {
  // 将纯文本 @用户名 改写为可点击的 GitHub 个人主页链接。
  final RegExp mentionPattern = RegExp(
    r'(?<!\[)(?<![\w./-])@([A-Za-z0-9](?:[A-Za-z0-9-]{0,38}))\b',
  );
  return markdown.replaceAllMapped(mentionPattern, (Match match) {
    final String userName = match.group(1)!;
    return '[@$userName](https://github.com/$userName)';
  });
}

String _rewriteGitHubCompareLinks(String markdown) {
  // 将 compare 裸链接转换为 [旧版本...新版本](url) 形式。
  final RegExp comparePattern = RegExp(
    r'(?<!\]\()<?(https://github\.com/[^/\s<>]+/[^/\s<>]+/compare/([^\s<>]+?\.\.\.[^\s<>]+))>?',
    caseSensitive: false,
  );
  return markdown.replaceAllMapped(comparePattern, (Match match) {
    final String url = match.group(1)!;
    final String range = _tryDecodeUriComponent(match.group(2)!);
    return '[$range]($url)';
  });
}

String _rewriteGitHubPullRequestLinks(String markdown) {
  // 将 pull 裸链接转换为 [#编号](url) 形式，与 GitHub 自动文案一致。
  final RegExp pullPattern = RegExp(
    r'(?<!\]\()<?(https://github\.com/[^/\s<>]+/[^/\s<>]+/pull/(\d+)(?:[^\s<>)]*)?)>?',
    caseSensitive: false,
  );
  return markdown.replaceAllMapped(pullPattern, (Match match) {
    final String url = match.group(1)!;
    final String prNumber = match.group(2)!;
    return '[#$prNumber]($url)';
  });
}

String _rewriteGitHubShorthandPullReferences(
  String markdown, {
  String? releasePageUrl,
}) {
  // 将类似 “in #4” 的简写转换为可点击链接，贴近 GitHub 发布页显示。
  final String repoBaseUrl = _resolveGitHubRepoBaseUrl(releasePageUrl);
  final RegExp shortPullPattern = RegExp(r'(?<!\]\()(?<!\[)(?<!\w)#(\d+)\b');
  return markdown.replaceAllMapped(shortPullPattern, (Match match) {
    final String pullNumber = match.group(1)!;
    return '[#$pullNumber]($repoBaseUrl/pull/$pullNumber)';
  });
}

String _resolveGitHubRepoBaseUrl(String? releasePageUrl) {
  // 优先从 release 页面 URL 解析 owner/repo，避免仓库名硬编码错误。
  final Uri? releaseUri = Uri.tryParse(releasePageUrl ?? '');
  if (releaseUri != null &&
      releaseUri.scheme == 'https' &&
      releaseUri.host.toLowerCase() == 'github.com' &&
      releaseUri.pathSegments.length >= 2) {
    final String owner = releaseUri.pathSegments[0];
    final String repo = releaseUri.pathSegments[1];
    if (owner.isNotEmpty && repo.isNotEmpty) {
      return 'https://github.com/$owner/$repo';
    }
  }
  return 'https://github.com/Lulozi/DormDevise';
}

String _tryDecodeUriComponent(String value) {
  // 尝试把 URL 编码片段恢复成可读文本，失败时保持原值。
  try {
